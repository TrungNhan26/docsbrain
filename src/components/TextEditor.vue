...
const sendContentToBackend = async (htmlContent) => {
  const response = await fetch('http://localhost:8000/create-doc-ai', {
    method: 'POST',
    headers: {
...
const saveOrUpdateDocument = async () => {
  try {
    ...
    const response = await axios.post('http://localhost:8000/documents/save', formData, {
      headers: {
...
const loadDocumentHistory = async () => {
  ...
  try {
    const token = localStorage.getItem('token');
    const response = await axios.get(`http://localhost:8000/documents/${documentId.value}/history`, {
      headers: {
        'Authorization': `Bearer ${token}`
      }
    });
...
const restoreFromHistory = async (historyId) => {
  try {
    ...
    const response = await axios.get(
      `http://localhost:8000/documents/history/content/${historyId}`,
      {
        headers: {
          'Authorization': `Bearer ${token}`
        }
