...
const fetchUserDocuments = async () => {
  ...
  try {
    const response = await axios.get(`http://localhost:8000/documents/user/${userId}`, {
...
const openDocument = async (docId) => {
  ...
  try {
    const response = await axios.get(`http://localhost:8000/documents/${docId}/content`, {
      headers: { 'Authorization': `Bearer ${token}` }
    });
...
const deleteDocumentConfirmed = async () => {
  ...
  try {
    await axios.delete(`http://localhost:8000/documents/${documentToDelete.value.id}`, {
      headers: { 'Authorization': `Bearer ${token}` }
    });
...
const triggerOpenHistoryModal = async (doc) => {
  ...
  try {
      const response = await axios.get(`http://localhost:8000/documents/${doc.id}/history`, {
          headers: { 'Authorization': `Bearer ${token}` }
      });
...
const restoreFromHistory = async (historyId, mainDocumentId) => {
    ...
    const token = localStorage.getItem('token');
    try {
        await axios.post(`http://localhost:8000/documents/restore/${historyId}`, 
